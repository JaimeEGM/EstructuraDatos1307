...
public class Convertidor {
	...
	public static String convertirApostfija(String interfija) {// se crea el metodo convertirApostfija y se le ingresa
																// la operacion infija como argumento
		String postfija = "";
		IntPost pila = new IntPost(30);

		for (int i = 0; i < interfija.length(); i++) {// se recorre la operacion infija para hacer la conversion. se
														// detendra cuando haya recorrido cada
			char letra = interfija.charAt(i); // elemento de la operacion infija
			if (esOperador(letra)) {
				if (letra == ')') {
